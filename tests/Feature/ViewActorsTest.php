...
class ViewActorsTest extends TestCase
{
    ...
    public function the_index_page_shows_the_correct_info()
    {
        ...
        Http::fake([
            'https://api.themoviedb.org/3/person/popular?language=ru-RU&page=1' => $this->fakePopularActors()
        ]);

        $response = $this->get(route('actors.index'));

        $response->assertSuccessful();
        $response->assertSee('Популярные актеры');
    ...
    {
        Http::fake([
            'https://api.themoviedb.org/3/person/popular?language=ru-RU&page=2' => $this->fakePopularActorsPage2(),
        ]);
    ...
    {
        Http::fake([
            'https://api.themoviedb.org/3/person/2888?language=ru-RU' => $this->fakeActor(),
            'https://api.themoviedb.org/3/person/2888/external_ids?language=ru-RU' => $this->fakeSocial(),
            'https://api.themoviedb.org/3/person/2888/combined_credits?language=ru-RU' => $this->fakeCredits(),
        ]);

        $response = $this->get(route('actors.show', 2888));
        $response->assertSuccessful();
        $response->assertSee('Will Smith');
        $response->assertSee('25.09.1968');
        $response->assertSee('Philadelphia, Pennsylvania, U.S.A.');
        $response->assertSee('The Pursuit of Happiness');
        ...
        return Http::response([
            'cast' => [
                ...
                [
                    ...
                    "original_title" => "The Pursuit of Happiness",
